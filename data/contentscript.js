var SlideEliminator = function(config) {
    	this.storage = config.storage;
		this.scrollableImageContainer = config.storage.scrollableImageContainer;
		this.spinningIconUrl = config.spinnerImgUrl;
		this.cssPath = config.cssUrl;
        this.fbRef = "ff.extension";
        this.facebookIconUrl = config.facebookIconUrl;
    ...
    function eliminateSlides() {
		 ...
		 * FF specific code
		 */
        this.applyFfSpecificCss = function(){
            /* hack dla FF */
		...
		this.canRunOnCurrentUrl = function() {
			var canRunHere = false;
			console.log(self.storage);
			$.each(self.storage, function(allowedHost, enabled) {
		...
		this.canRunOnCurrentUrl();
	};

self.on('message', function(config) {
	new SlideEliminator(config);
});